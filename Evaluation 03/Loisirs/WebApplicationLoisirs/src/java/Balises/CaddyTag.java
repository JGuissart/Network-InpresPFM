 ...
 * @author adrie
 */
public class CaddyTag extends BodyTagSupport
{
     ...
     */
    private void otherDoStartTagOperations() throws SQLException
    {
    ...
    {
        HashMap<Integer,Produits> caddy = (HashMap<Integer,Produits>)pageContext.getSession().getAttribute("\""+bodyContent.getString()+"\"");
        
        for(HashMap.Entry<Integer,Produits> entry : caddy.entrySet())
        {
            try
            {
                ResultSet rs = bean.Select("*","produits","idProduit='"+ entry.getValue().getIdProduit()+"'");

               out.print("<h4>Nom :" + rs.getString("nom")+ "</h4>");
               out.print("<h4>Quantité restante :" + rs.getInt("quantite")+ "</h4>");
               out.print("<h4>Prix :" + rs.getInt("prix")+ "</h4>");
    ...
    public int doStartTag() throws JspException
    {
        ...
        {
            Logger.getLogger(CaddyTag.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void handleBodyContentException(Exception ex) throws JspException
    {
        ...
        throw new JspException("Error in CaddyTag tag", ex);
    }
